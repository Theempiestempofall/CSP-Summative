...
shape_num_edges = trtl.textinput("Number", "Pick an integer between 3 and 1000 (inclusive)")
while shape_num_edges is None or not shape_num_edges.isdigit() or not (3 <= int(shape_num_edges) <= 1000):
    shape_num_edges = trtl.textinput("Number", "Please pick an integer between 3 and 1000 (inclusive)")
shape_edges_list = []
# remove random edges from the list to create a unique shape
num_adds = max(int(shape_num_edges), 3)
for i in range(num_adds):
    rand_point_x = random.randint(-300, 300)
    rand_point_y = random.randint(-300, 300)
